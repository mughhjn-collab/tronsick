...
function doClaim(){const btn=document.getElementById('claimBtn'),note=document.getElementById('claimNote');btn.disabled=true;btn.textContent='Processing...';setTimeout(()=>{const amt=(Math.random()*.004+.001).toFixed(6);addBal(parseFloat(amt));note.textContent='Claimed '+amt+' TRX!';note.style.color='#3ecf8e';btn.textContent='CLAIMED!';document.getElementById('capChk').checked=false;localStorage.setItem('lastClaim',Date.now().toString());setTimeout(()=>startClaimCountdown(1800),1500);},1200);}
let rollsLeft=0;
...
function dmVerify(){var b=dmBetHistory[window._dmVerifyIdx||0];localStorage.setItem('dgVerifyData',JSON.stringify({game:'Diamond',clientSeed:clientSeed,serverSeedHash:serverSeedHash,serverSeed:serverSeed,nonce:dmNonce,bet:b||null,bets:dmBetHistory.slice(0,50)}));window.open('verify.html','_blank');}
// END DIAMOND GAME
// ==========================================
// SIC BO GAME
// ==========================================
...
function wlVerify(){var b=wlBetHistory[window._wlVerifyIdx||0];localStorage.setItem('dgVerifyData',JSON.stringify({game:'Wheel',clientSeed:clientSeed,serverSeedHash:serverSeedHash,serverSeed:serverSeed,nonce:wlNonce,bet:b||null,bets:wlBetHistory.slice(0,50)}));window.open('verify.html','_blank');}
// ==========================================
// END WHEEL GAME
// ==========================================
...
'<div class="bm-sf"><div class="bm-sf-lbl">Client seed</div><div class="bm-sf-row"><span class="bm-sf-ico">&#9000;</span><input class="bm-sf-inp" readonly value="'+b.cs+'"></div></div>'+
'<div class="bm-sf bm-sf-nonce"><div class="bm-sf-lbl">Nonce</div><div class="bm-sf-row"><span class="bm-sf-ico">#</span><input class="bm-sf-inp" readonly value="'+b.id+'"></div></div>';
var vbtn=document.getElementById('bmVerifyBtn');if(vbtn){vbtn.onclick=function(){twVerify(i);};}
modal.style.display='flex';
}

function showSbBetModal(i){
window._sbVerifyIdx=i;
...
'<div class="bm-sf"><div class="bm-sf-lbl">Client seed</div><div class="bm-sf-row"><span class="bm-sf-ico">&#9000;</span><input class="bm-sf-inp" readonly value="'+b.cs+'"></div></div>'+
'<div class="bm-sf bm-sf-nonce"><div class="bm-sf-lbl">Nonce</div><div class="bm-sf-row"><span class="bm-sf-ico">#</span><input class="bm-sf-inp" readonly value="'+b.id+'"></div></div>';
var vbtn=document.getElementById('bmVerifyBtn');if(vbtn){vbtn.onclick=function(){sbVerify(i);};}
modal.style.display='flex';
}
function sbVerify(i){
var b=sbBetHistory[i]||sbBetHistory[window._sbVerifyIdx||0];if(!b)return;
var url='verify.html?game=sicbo&seed='+encodeURIComponent(b.sv)+'&hash='+encodeURIComponent(b.ssh)+'&client='+encodeURIComponent(b.cs)+'&nonce='+b.id;
window.open(url,'_blank');
}
function twVerify(i){
var b=twBetHistory[i]||twBetHistory[window._twVerifyIdx||0];if(!b)return;
var url='verify.html?game=tower&seed='+encodeURIComponent(b.sv)+'&hash='+encodeURIComponent(b.ssh)+'&client='+encodeURIComponent(b.cs)+'&nonce='+b.id+'&mode='+b.mode;
window.open(url,'_blank');
}
