...
function affClaim(){
  var clEl=document.getElementById('affClaimable');
  var amt=parseFloat((clEl||{}).textContent||'0');
  if(amt<=0){showToast('No earnings to claim.');return;}
  var btn=document.querySelector('.aff-claim-btn');
  if(btn){btn.disabled=true;btn.textContent='Claiming...';}
  setTimeout(function(){
    addBal(amt);
    // Zero out claimable
    if(clEl) clEl.textContent='0.000000';
    var tEl=document.getElementById('affTotalEarned'); if(tEl) tEl.textContent='0.000000';
    // Store claimed amount
    try{
      var uname=(localStorage.getItem('userName')||'').toLowerCase();
      var prev=parseFloat(localStorage.getItem('affTotalClaimed_'+uname)||'0');
      localStorage.setItem('affTotalClaimed_'+uname,(prev+amt).toFixed(6));
    }catch(e){}
    showToast('Claimed +'+amt.toFixed(6)+' TRX!');
...
function initNewUserBonus(){
  ...
  if(!localStorage.getItem(_bonusKey)){
    ...
    localStorage.setItem(_rollsKey,'0');
    rollsLeft=0;
    if(rc)rc.textContent=rollsLeft;
    if(note){note.textContent='No bonus rolls. Use Lucky Draw to play!';note.style.color='rgba(232,240,235,.4)';}
...
function mnRefreshGrid(){for(var i=0;i<25;i++){var cell=document.getElementById('mnCell'+i);if(!cell)continue;var g=mnGrid[i];if(g.revealed){if(g.mine){cell.className='mn-cell mn-mine';cell.innerHTML='&#128163;';}else{cell.className='mn-cell mn-safe';cell.innerHTML='&#128142;';}}else{cell.className='mn-cell mn-hidden';cell.textContent='';}}}
function mnBetStart(){if(mnActive)return;var bet=parseFloat((document.getElementById('mnAmt')||{}).value)||0;var bal=parseFloat(document.getElementById('userBalance').textContent)||0;if(bet<=0){showToast('Enter bet!');return;}if(bal<bet){showToast('Insufficient balance!');return;}mnBet=bet;addBal(-bet);updateWager(bet);mnNonce++;mnActive=true;mnGrid=[];for(var i=0;i<25;i++)mnGrid.push({mine:false,revealed:false});var pos=[];for(var j=0;j<25;j++)pos.push(j);pos.sort(function(){return Math.random()-0.5;});for(var k=0;k<mnMines;k++)mnGrid[pos[k]].mine=true;mnPickCount=0;mnRefreshGrid();var btn=document.getElementById('mnBetBtn');if(btn){btn.disabled=false;btn.textContent='Pick a cell!';btn.onclick=function(){if(mnPickCount>0){mnCashOut();}else{showToast('Pick at least one cell first!');}};}var co=document.getElementById('mnCashOut');if(co)co.disabled=false;var mu=document.getElementById('mnMult');if(mu)mu.textContent='1.00x';var pc=document.getElementById('mnPickCnt');if(pc)pc.textContent='Picks: 0';}
function mnReveal(idx){if(!mnActive)return;var g=mnGrid[idx];if(g.revealed)return;if(!g.mine){var _mnAb=_abCheckWin(mnBet,50,mnCalcMult(Math.max(1,mnPickCount+1),mnMines));if(_mnAb===false)g.mine=true;else if(_mnAb===true&&g.mine)g.mine=false;}g.revealed=true;var cell=document.getElementById('mnCell'+idx);if(g.mine){cell.className='mn-cell mn-mine';cell.innerHTML='&#128163;';mnActive=false;mnGrid.forEach(function(c,i){if(c.mine)c.revealed=true;});mnRefreshGrid();var now=new Date();var ts=(now.getDate()<10?'0':'')+now.getDate()+'/'+(now.getMonth()<9?'0':'')+(now.getMonth()+1);mnBetHistory.unshift({id:mnNonce,mines:mnMines,picks:mnPickCount,bet:mnBet,win:false,mult:0,profit:-mnBet,ts:ts,cs:clientSeed,ssh:serverSeedHash,sv:serverSeed});try{localStorage.setItem('mnHistory',JSON.stringify(mnBetHistory.slice(0,50)));}catch(e){}mnRenderBets();showToast('BOOM! Mine hit!');var wa=document.getElementById('mnWinAmt');if(wa){wa.textContent='-'+mnBet.toFixed(6);wa.style.color='#ef4444';}var co=document.getElementById('mnCashOut');if(co)co.disabled=true;var btn=document.getElementById('mnBetBtn');if(btn){btn.disabled=false;btn.textContent='Place Bet';btn.onclick=mnBetStart;}setTimeout(function(){mnGrid.forEach(function(c){c.revealed=false;c.mine=false;});mnRefreshGrid();if(mnAutoRunning)mnAutoNext();},1400);}else{cell.className='mn-cell mn-safe';cell.innerHTML='&#128142;';mnPickCount++;var mult=mnCalcMult(mnPickCount,mnMines);var mu=document.getElementById('mnMult');if(mu)mu.textContent=mult.toFixed(2)+'x';var pc=document.getElementById('mnPickCnt');if(pc)pc.textContent='Picks: '+mnPickCount;var ca=document.getElementById('mnCashOutAmt');if(ca)ca.textContent=(mnBet*mult).toFixed(6);var safeLeft=mnGrid.filter(function(c){return !c.mine&&!c.revealed;}).length;if(safeLeft===0)mnCashOut();}}
function mnCashOut(){if(!mnActive)return;mnActive=false;var mult=mnCalcMult(mnPickCount,mnMines);var won=mnBet*mult;addBal(won);var profit=won-mnBet;var now=new Date();var ts=(now.getDate()<10?'0':'')+now.getDate()+'/'+(now.getMonth()<9?'0':'')+(now.getMonth()+1);mnBetHistory.unshift({id:mnNonce,mines:mnMines,picks:mnPickCount,bet:mnBet,win:true,mult:mult,profit:profit,ts:ts,cs:clientSeed,ssh:serverSeedHash,sv:serverSeed});try{localStorage.setItem('mnHistory',JSON.stringify(mnBetHistory.slice(0,50)));}catch(e){}mnRenderBets();showToast('Cashed out! +'+profit.toFixed(6)+' TRX');var wa=document.getElementById('mnWinAmt');if(wa){wa.textContent='+'+profit.toFixed(6);wa.style.color='#22c55e';}var co=document.getElementById('mnCashOut');if(co)co.disabled=true;var btn=document.getElementById('mnBetBtn');if(btn){btn.disabled=false;btn.textContent='Place Bet';btn.onclick=mnBetStart;}setTimeout(function(){mnGrid.forEach(function(c){c.revealed=false;c.mine=false;});mnRefreshGrid();if(mnAutoRunning)mnAutoNext();},600);}
...
function cfRenderHist(){
  ...
  body.innerHTML = cfBetHistory.slice(0,20).map(function(b,i){
    return '<tr class="dg-hist-row" style="cursor:pointer" onclick="cfOpenBetInfo('+i+')">'+
      ...
      '<td><button class="dg-info-btn" onclick="event.stopPropagation();cfOpenBetInfo('+i+')">?</button></td>'+
    '</tr>';
  }).join('');
}
...
var _SITE_LANGS = {
  ...
  id: { claim:'KLAIM', faucet:'Faucet', games:'Game', deposit:'Setor', withdraw:'Tarik',
        ...
        langLabel:'Bahasa' },
  hi: { claim:'???? ????', faucet:'?????', games:'?????', deposit:'???', withdraw:'??????',
        surveys:'?????????', affiliates:'?????', gifts:'?????', settings:'????????',
        contact:'??????', cashback:'??????', contest:'???????????', home:'???', logout:'??????',
        welcome:'?????? ??', balance:'??????', level:'???? ????', stake:'????????',
        langLabel:'????' },
  ur: { claim:'???? ????', faucet:'?????', games:'????', deposit:'???', withdraw:'?????',
        surveys:'????', affiliates:'?????', gifts:'?????', settings:'???????',
        contact:'?????', cashback:'??? ???', contest:'??????', home:'???', logout:'??? ???',
        welcome:'??? ?????', balance:'?????', level:'?? ?? ???', stake:'???????',
        langLabel:'????' },
  ...
  sw: { claim:'DAI', faucet:'Bomba', games:'Michezo', deposit:'Weka', withdraw:'Toa',
        ...
        langLabel:'Lugha' },
  zh: { claim:'??', faucet:'???', games:'??', deposit:'??', withdraw:'??',
        surveys:'??', affiliates:'??', gifts:'???', settings:'??',
        contact:'??', cashback:'??', contest:'??', home:'??', logout:'??',
        welcome:'????', balance:'??', level:'????', stake:'??',
        langLabel:'??' },
  ja: { claim:'????', faucet:'??????', games:'???', deposit:'??', withdraw:'??',
        surveys:'?????', affiliates:'???????', gifts:'???', settings:'??',
        contact:'??????', cashback:'????????', contest:'?????', home:'???', logout:'?????',
        welcome:'???????', balance:'??', level:'???????', stake:'??????',
        langLabel:'??' },
  ko: { claim:'??', faucet:'????', games:'??', deposit:'??', withdraw:'??',
        surveys:'????', affiliates:'??', gifts:'??', settings:'??',
        contact:'???', cashback:'???', contest:'??', home:'?', logout:'????',
        welcome:'?????', balance:'??', level:'? ??', stake:'????',
        langLabel:'??' },
  ...
  th: { claim:'???', faucet:'???????', games:'?????', deposit:'???', withdraw:'???',
        surveys:'????????', affiliates:'????????', gifts:'???????????', settings:'???????',
        contact:'??????', cashback:'???????', contest:'??????????', home:'????????', logout:'??????????',
        welcome:'????????????', balance:'??????????', level:'???????????', stake:'Staking',
        langLabel:'????' },
  fa: { claim:'??????', faucet:'???', games:'???????', deposit:'?????', withdraw:'??????',
        surveys:'???????', affiliates:'??????', gifts:'???? ????', settings:'???????',
        contact:'????', cashback:'?????? ???', contest:'??????', home:'????', logout:'????',
        welcome:'??? ?????', balance:'??????', level:'??? ???', stake:'??????????',
        langLabel:'????' },
  ...
  ha: { claim:'KARBA', faucet:'Famfo', games:'Wasanni', deposit:'Saka', withdraw:'Fita',
        ...
        contact:'Tuntu?a', cashback:'Dawo', contest:'Gasar', home:'Gida', logout:'Fita',
        welcome:'Barka da zuwa', balance:'Asusun', level:'Matakinka', stake:'Staking',
        langLabel:'Harshe' },
  yo: { claim:'GBA', faucet:'Faucet', games:'Ere', deposit:'Fi sii', withdraw:'Y? kuro',
        surveys:'Iwadi', affiliates:'Alaba?i??', gifts:'?bun', settings:'Eto',
        contact:'Kan si', cashback:'Ipadab?', contest:'Idije', home:'Ile', logout:'Jade',
        welcome:'E kaabo', balance:'Owo', level:'Ipele r?', stake:'Staking',
        langLabel:'Ede' },
  bn: { claim:'???? ????', faucet:'????', games:'????', deposit:'???', withdraw:'???????',
        surveys:'????', affiliates:'????????????', gifts:'?????', settings:'??????',
        contact:'???????', cashback:'??????????', contest:'???????????', home:'???', logout:'?????',
        welcome:'???????', balance:'?????????', level:'????? ????', stake:'???????',
        langLabel:'????' }
